...
public class Result<T> implements Serializable {
	...
	public static <T> Result<T> ERROR(StatefulException statefulException, T data) {
		if (Objects.nonNull(statefulException)) {
			return Result.ERROR(statefulException.getStatus(), statefulException.getMessage(), data);
		} else {
	...
	public static <T> Result<T> ERROR(StatefulException statefulException) {
		if (Objects.nonNull(statefulException)) {
			return Result.ERROR(statefulException.getStatus(), statefulException.getMessage(), null);
		} else {
			return Result.ERROR(500, "服务器异常", null);
		}
	}
}
